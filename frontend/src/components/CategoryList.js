...
export default function CategoryList() {
  ...
  useEffect(() => {
    const token = Cookies.get('token'); // Usando js-cookie para acessar o token do cookie
    if (!token) {
    ...
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await axios.delete(`http://localhost:3335/api/v1/categories/${id}`, {
          headers: {
            Authorization: `Bearer ${Cookies.get('token')}` // Passando o token do cookie para o header
          }
        });
        fetchCategories();
      } catch (error) {
